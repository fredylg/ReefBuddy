...
function payloadFor(
  over: Partial<FakePayload> & { transactionId: string; environment: FakePayload['environment'] }
): FakePayload {
  return {
...
beforeAll(async () => {
  signer = (await crypto.subtle.generateKey({ name: 'ECDSA', namedCurve: 'P-256' }, true, [
    'sign',
    'verify',
  ])) as CryptoKeyPair;
...
async function selfSignedJWS(payload: FakePayload): Promise<string> {
  ...
  const sig = new Uint8Array(
    await crypto.subtle.sign({ name: 'ECDSA', hash: 'SHA-256' }, signer.privateKey, signingInput)
  );
...
describe('POST /credits/purchase — request validation', () => {
  ...
  it('rejects an unknown product', async () => {
    const jws = await selfSignedJWS(
      payloadFor({ transactionId: uid('tx'), environment: 'Xcode', productId: 'com.reefbuddy.unknown' })
    );
...
describe('POST /credits/purchase — signature is verified before anything is trusted (P1-01)', () => {
  it('rejects a forged, unsigned Sandbox payload and grants nothing', async () => {
    ...
    const res = await purchase({
      ...
      jwsRepresentation: forgedJWS(
        payloadFor({ transactionId: uid('tx'), environment: 'Sandbox', productId: 'com.reefbuddy.credits50' })
      ),
    });
    expect(res.status).toBe(400);
  ...
  it('rejects a correctly signed JWS whose signed product differs from the requested product', async () => {
    const deviceId = uid('dev');
    const jws = await selfSignedJWS(
      payloadFor({ transactionId: uid('tx'), environment: 'Xcode', productId: PRODUCT_5 })
    );
  ...
  it('rejects a signed JWS for another bundle id', async () => {
    const deviceId = uid('dev');
    const jws = await selfSignedJWS(
      payloadFor({ transactionId: uid('tx'), environment: 'Xcode', bundleId: 'com.example.other' })
    );
...
describe('POST /credits/purchase — granting and duplicate guard (P1-02, P1-06)', () => {
  ...
  it('the genuine Sandbox transaction replayed by another device is a duplicate', async () => {
    ...
    const a = uid('dev');
    const b = uid('dev');
    expect((await purchase({ deviceId: a, productId: PRODUCT_5, jwsRepresentation: appleSandboxJws })).status).toBe(
      200
    );
    expect((await purchase({ deviceId: b, productId: PRODUCT_5, jwsRepresentation: appleSandboxJws })).status).toBe(
      409
    );
    expect((await balance(b)).paidCredits).toBe(0);
  });
  ...
  it('transaction id "0" is no longer special: the second "0" transaction is a duplicate', async () => {
    await env.DB.prepare("DELETE FROM purchase_history WHERE apple_transaction_id = '0'").run();
    const deviceId = uid('dev');
    const first = await selfSignedJWS(
      payloadFor({ transactionId: '0', environment: 'Xcode', purchaseDate: Date.now() - 1000 })
    );
  ...
  it('every granted purchase has an audit row in purchase_history', async () => {
    ...
    const row = await env.DB.prepare(
      'SELECT credits_added, device_id FROM purchase_history WHERE apple_transaction_id = ?'
    )
      .bind(txId)
