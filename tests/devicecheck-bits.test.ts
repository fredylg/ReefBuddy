...
mockOrigin('https://api.devicecheck.apple.com', async (request) => {
  ...
  if (!request.headers.get('Authorization')?.startsWith('Bearer '))
    return mockJsonResponse(401, 'Unable to verify authorization token');
  if (!body.device_token || body.device_token.length < 500)
    return mockJsonResponse(400, 'Missing or incorrectly formatted device token payload');
  if (path.endsWith('/query_two_bits')) {
    if (queryStatus !== 200)
      return mockJsonResponse(queryStatus, 'Missing or incorrectly formatted device token payload');
    return deviceBits
      ? mockJsonResponse(200, { ...deviceBits, last_update_time: '2026-09' })
      : mockJsonResponse(200, 'Failed to find bit state');
  }
