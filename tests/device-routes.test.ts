...
async function call(
  ...
  path: string,
  body?: unknown,
  deviceId: string | null = deviceA
): Promise<Response> {
  ipCounter++;
  const headers: Record<string, string> = {
    'Content-Type': 'application/json',
    'CF-Connecting-IP': `10.7.${(ipCounter >> 8) & 255}.${ipCounter & 255}`,
  };
...
describe('measurements and history', () => {
  ...
  it('POST /api/measurements stores nitrite and PPT salinity', async () => {
    const res = await call('POST', '/api/measurements', {
      tank_id: tankId.toLowerCase(),
      ph: 8.1,
      nitrite: 0.1,
      salinity: 35,
      salinity_unit: 'PPT',
  ...
  it('GET /tanks/:id/history returns the measurement with nitrite and notes', async () => {
    const res = await call(
      'GET',
      `/tanks/${tankId}/history?start=${encodeURIComponent(isoAgo(1))}&end=${encodeURIComponent(isoNow())}`
    );
    expect(res.status).toBe(200);
  ...
  it('GET /tanks/:id/export returns CSV with nitrite and notes columns', async () => {
    const res = await call(
      'GET',
      `/tanks/${tankId}/export?start=${encodeURIComponent(isoAgo(1))}&end=${encodeURIComponent(isoNow())}`
    );
    expect(res.status).toBe(200);
...
describe('livestock via /api', () => {
  ...
  it('POST /api/tanks/:id/livestock', async () => {
    const res = await call('POST', `/api/tanks/${tankId}/livestock`, {
      name: 'Clownfish',
  ...
  it("PUT /api/livestock/:id accepts the app's health statuses", async () => {
    expect((await call('PUT', `/api/livestock/${livestockId}`, { quantity: 3, healthStatus: 'thriving' })).status).toBe(
      200
    );
  ...
  it('POST /api/tanks/:id/livestock accepts Anemone and Other categories', async () => {
    const res = await call('POST', `/api/tanks/${tankId}`.replace(/$/, '/livestock'), {
      name: 'BTA',
    ...
    expect([200, 201]).toContain(res.status);
    const other = await call('POST', `/api/tanks/${tankId}/livestock`, {
      name: 'Mystery snail',
      category: 'Other',
      quantity: 1,
    });
  ...
  it('POST + GET /api/livestock/:id/logs', async () => {
    expect([200, 201]).toContain(
      (await call('POST', `/api/livestock/${livestockId}/logs`, { logType: 'feeding', description: 'Mysis' })).status
    );
  ...
  it('DELETE /api/livestock/:id, then it is gone', async () => {
    ...
    expect((await call('GET', `/api/tanks/${tankId}/livestock`).then((r) => r.text())).toLowerCase()).not.toContain(
      livestockId.toLowerCase()
    );
  });
});
...
describe('maintenance schedules and water changes', () => {
  ...
  it('water change linked to the schedule, listed, then deleted', async () => {
    const created = await call('POST', `/api/tanks/${tankId}/water-changes`, {
      percentReplaced: 10,
      sourceScheduleId: scheduleId.toLowerCase(),
    });
