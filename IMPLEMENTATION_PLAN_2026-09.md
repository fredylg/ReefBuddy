...
- [x] **P7-05** (X-04) copy + policy done 2026-09-07; addresses moved to aethers.com.au and routed 2026-09-08 (owner: send one test mail) — `web/`: soften Smart Alerts copy; privacy policy data inventory + date; 🧑 confirm `privacy@`/`support@reefbuddy.app` exist.
...
- 2026-09-08 · signing · project moved to automatic signing with team `UAUBPV3Q4Z` after the manual/empty-team setup blocked archiving; archive + App Store export verified from the command line; IPA `~/Downloads/ReefBuddy-1.0.8-build7.ipa` · 6e458fe
- 2026-09-08 · P4-28 / P6-08 · **TestFlight 1.0.8 build 7 tested on the owner's iPhone against production**: app works, one real analysis completed (owner balance 11 → 10 paid credits, 63 analyses). Remaining: App Store submission, then `workers_dev = false`.
- 2026-09-08 · P7-05 follow-up · owner does not own reefbuddy.app; site emails changed to `privacy-reefbuddy@`, `support-reefbuddy@`, `legal-reefbuddy@aethers.com.au` (web/ committed, **website not redeployed yet** — needs `npm run deploy:web` go-ahead). Mailboxes/aliases still to be created by the owner.
